...
st.set_page_config(page_title="품질 변화 분석기", page_icon="📊", layout="wide")
...
# ── 메인 ─────────────────────────────────────────────────────────────────────
st.markdown("## 📊 품질 변화 분석기")

# ── 데이터 소스: GitHub 자동 로드 or 업로드 ──────────────────────────────────
github_bytes = fetch_from_github()

if github_bytes:
    st.success("✅ 데이터 자동 로드됨 (모바일에서도 동일하게 적용됩니다)")
    with st.expander("📂 데이터 업데이트 (새 파일로 교체)"):
        ...
        if uploaded:
            ...
            with st.spinner("GitHub에 저장 중..."):
                ok = commit_to_github(file_bytes)
            if ok:
                fetch_from_github.clear()
                st.success("✅ 업로드 완료! 잠시 후 모든 기기에 반영됩니다.")
                st.rerun()
            else:
                st.error("❌ 저장 실패 — GITHUB_TOKEN을 확인해 주세요.")
    file_source = github_bytes
else:
    st.info("📂 저장된 데이터가 없습니다. 엑셀 파일을 업로드해 주세요.")
    uploaded = st.file_uploader("품질 데이터 엑셀 파일 업로드 (.xlsx)", type=["xlsx"])
    if not uploaded:
        st.stop()
    file_bytes = uploaded.getvalue()
    with st.spinner("GitHub에 저장 중..."):
    ...
    if ok:
        fetch_from_github.clear()
        st.success("✅ 업로드 완료! 이제 모바일에서도 자동으로 로드됩니다.")
        st.rerun()
    file_source = file_bytes

with st.spinner("데이터 로딩 중..."):
    df = load_data(io.BytesIO(file_source) if isinstance(file_source, bytes) else file_source)
