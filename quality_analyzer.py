...
def generate_feedback(result: pd.DataFrame, base_label: str, compare_label: str) -> str:
    if result.empty:
        return ""

    critical = result[result["둘다 악화"]]
    bad_only  = result[result["기준대비 악화"] & ~result["둘다 악화"]]
    trend_only = result[result["추세 악화"] & ~result["기준대비 악화"]]
    good      = result[~result.apply(is_bad, axis=1)]

    base_col    = [c for c in result.columns if "기준(" in c][0]
    compare_col = [c for c in result.columns if "비교(" in c][0]

    lines = []
    lines.append(f"**📋 품질 분석 피드백**")
    lines.append(f"> 기준일 **{base_label}** → 비교일 **{compare_label}** 비교")
    lines.append("")

    total = len(result)
    n_bad = len(result[result.apply(is_bad, axis=1)])
    if n_bad == 0:
        lines.append("✅ **전체 KPI가 양호합니다.** 특이 사항 없음.")
        return "\n".join(lines)

    lines.append(f"총 **{total}개** KPI 중 **{n_bad}개** 항목에서 품질 저하가 확인됩니다.")
    lines.append("")

    # 즉시 점검 필요 (둘다 악화)
    if not critical.empty:
        lines.append("---")
        lines.append("### 🚨 즉시 점검 필요")
        for _, row in critical.iterrows():
            v1, v2 = row[base_col], row[compare_col]
            delta  = row["변화량"]
            dp     = row["변화율(%)"]
            unit   = row["단위"]
            dp_str = f"{dp:+.1f}%" if isinstance(dp, float) else ""
            direction = "하락" if row["higher_better"] else "상승"
            lines.append(
                f"- **{row['KPI']}**: {v1}{unit} → {v2}{unit} "
                f"({dp_str}, {direction}) — 지속적인 악화 추세 확인"
            )
    ...
    if not bad_only.empty:
        lines.append("")
        lines.append("---")
        lines.append("### ⚠️ 기준일 대비 악화")
        for _, row in bad_only.iterrows():
            v1, v2 = row[base_col], row[compare_col]
            dp     = row["변화율(%)"]
            unit   = row["단위"]
            dp_str = f"{dp:+.1f}%" if isinstance(dp, float) else ""
            lines.append(
                f"- **{row['KPI']}**: {v1}{unit} → {v2}{unit} ({dp_str}) — 추세 지속 여부 모니터링 필요"
            )
    ...
    if not trend_only.empty:
        lines.append("")
        lines.append("---")
        lines.append("### 📉 하락 추세 진행 중")
        for _, row in trend_only.iterrows():
            unit = row["단위"]
            v2   = row[compare_col]
            lines.append(
                f"- **{row['KPI']}**: 현재 {v2}{unit} — 수치는 유지되나 서서히 나빠지는 중"
            )
    ...
    if not good.empty:
        lines.append("")
        lines.append("---")
        good_names = ", ".join(good["KPI"].tolist())
        lines.append(f"✅ **정상 유지**: {good_names}")
...
def make_chart(daily, col, kpi_name, unit, color="#3498db"):
    ...
    fig.update_layout(
        title=dict(
            text=f"{kpi_name} ({unit})" if unit else kpi_name,
            font=dict(size=12, color="#111111"),
        ),
        height=220,
        margin=dict(l=50, r=10, t=38, b=42),
        showlegend=False,
        plot_bgcolor="#f8f9fa",
        paper_bgcolor="white",
        xaxis=dict(
...
def render_html_table(rows: list[dict]) -> None:
    ...
    for idx, r in enumerate(rows):
        row_bg = "background:#2a2a2a;" if idx % 2 == 0 else "background:#1e1e1e;"
        tds = ""
        for h in headers:
            v = r[h]
            if h == "상태":
                if "심각" in str(v):   color = "color:#ff6b6b;font-weight:700;"
                elif "악화" in str(v): color = "color:#ffa94d;font-weight:700;"
                elif "하락" in str(v): color = "color:#ffd43b;font-weight:700;"
                else:                   color = "color:#69db7c;font-weight:700;"
            else:
                color = "color:#f0f0f0;"
            tds += (
                f"<td style='padding:8px 7px;border-bottom:1px solid #444;"
                f"font-size:0.83rem;white-space:nowrap;{row_bg}{color}'>{v}</td>"
            )
        body += f"<tr>{tds}</tr>"

    th_style = "padding:8px 7px;background:#333333;color:#ffffff;border-bottom:2px solid #555;font-size:0.79rem;white-space:nowrap;text-align:left;"
    th = "".join(f"<th style='{th_style}'>{h}</th>" for h in headers)

    html = (
        "<div style='overflow-x:auto;-webkit-overflow-scrolling:touch;"
        "max-height:420px;overflow-y:auto;border-radius:6px;border:1px solid #444;'>"
...
with tab1:
    ...
    # ── 자동 피드백 ──────────────────────────────────────────────────────────
    st.markdown("---")
    feedback = generate_feedback(result, base_label, compare_label)
...
st.caption(
    ...
    f"Cell: **{'전체' if (not cell_options or all_cells_check) else str(sorted(selected_cells))}** | "
