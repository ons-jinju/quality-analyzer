...
if st.session_state.file_bytes is None:
    with st.spinner("데이터 불러오는 중..."):
        st.session_state.file_bytes = fetch_from_github()

if st.session_state.file_bytes:
    # 데이터 있음 → 업데이트 옵션만 제공
    col_msg, col_btn = st.columns([6, 1])
    col_msg.success("✅ 데이터 로드됨 — 모바일에서도 동일하게 적용됩니다.")
    ...
    with st.expander("📂 데이터 업데이트 (새 파일로 교체)"):
        ...
        if uploaded:
            ...
            if ok:
                ...
                st.success("✅ 업로드 완료! 모든 기기에 반영됩니다.")
            else:
                st.error("❌ 저장 실패 — Secrets의 GITHUB_TOKEN을 확인해 주세요.")
else:
    # 데이터 없음 → 업로드 요청 또는 재시도
    st.warning("📂 저장된 데이터가 없거나 불러오지 못했습니다.")
    col1, col2 = st.columns(2)
    if col1.button("🔄 다시 불러오기"):
        st.session_state.file_bytes = None
        st.rerun()

    with col2:
        uploaded = st.file_uploader("엑셀 파일 업로드 (.xlsx)", type=["xlsx"], key="first_upload")
    if not uploaded:
        st.stop()
    raw = uploaded.getvalue()
    with st.spinner("GitHub에 저장 중..."):
...
with st.spinner("데이터 로딩 중..."):
    df = load_data(io.BytesIO(st.session_state.file_bytes))
